...
int left_most, right_most, up_most, down_most;
...
int getRandomX() {
	...
	return (rand() % (right_most - left_most + 1)) + left_most;

}

int getRandomY() {

	return (rand() % (up_most - down_most + 1)) + up_most;
...
int getAvailActNums(char** c_grid) {
	...
	for (int i = left_most; i <= right_most; i++) {
		...
		for (int j = up_most; j <= down_most; j++) {
...
Node* expand(Node* state) {
	...
	int n = getAvailActNums(state->grid);
	...
	while (true) {
		...
		//x = (rand() % (right_most-left_most+1))+left_most;

		y = rand() % (SIZE);

		// y = (rand() % (up_most-down_most+1))+up_most;

		if (state->grid[x][y] == SPACE) {

			int count = 0;

			for (int i = 0; i < state->child_number; i++) {

				if (state->child[i]->act->x == x && state->child[i]->act->y == y) {
...
Node* newNode(Node* state, double c) {
	...
	while (!isGridFull(c_state->grid) && !isWin(c_state->grid, c_state->act)) {
		...
		if (getAvailActNums(c_state->grid) > c_state->child_number) {
...
Action* mctsSearch(int times, char player) {
...
void offline_testing() {
	...
	while (true) {
		...
		mcts_act = mctsSearch(30000, player);
